...
Orpheus releases are shipped from `main` after the release candidate passes local checks and a basic device smoke test.
...
```sh
JAVA_HOME=/usr/lib/jvm/java-21-openjdk ./gradlew :app:assembleRelease -Porpheus.enableAbiSplits=true
```
