...
```sh
JAVA_HOME=/usr/lib/jvm/java-21-openjdk ./gradlew :app:dependencies --configuration releaseRuntimeClasspath > /tmp/opencode/orpheus-releaseRuntimeClasspath.txt
rg -o "[A-Za-z0-9_.-]+:[A-Za-z0-9_.-]+:[A-Za-z0-9_.+:-]+" /tmp/opencode/orpheus-releaseRuntimeClasspath.txt | sort -u | wc -l
```
