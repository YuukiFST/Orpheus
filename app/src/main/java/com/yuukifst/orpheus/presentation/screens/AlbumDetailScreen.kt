...
import com.yuukifst.orpheus.presentation.viewmodel.AlbumDetailViewModel
                ...
                Box(
                    ...
                    LazyColumn(
                        ...
                        songsByDisc.forEach { (discNumber, discSongs) ->
                            ...
                            itemsIndexed(
                                items = discSongs,
                                key = { _, song -> "album_song_${song.id}" },
...
private fun SharedAlbumTopBarProbe(
    ...
    val statusBarColor =
        if (LocalOrpheusDarkTheme.current) Color.Black.copy(alpha = 0.6f)
    ...
    OrpheusStatusBarStyle(color = fallbackStatusBarColor)
...
private fun CollapsingAlbumTopBar(
    ...
    val surfaceColor = MaterialTheme.colorScheme.surface
    val statusBarColor =
        if (LocalOrpheusDarkTheme.current) Color.Black.copy(alpha = 0.6f) else Color.White.copy(
            alpha = 0.4f
        )
