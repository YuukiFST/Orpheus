...
import com.yuukifst.orpheus.data.repository.NoLyricsFoundException
...
import com.yuukifst.orpheus.utils.isYouTubeSource
import java.io.File
...
class LyricsStateHolder @Inject constructor(
    ...
    fun initialize(
        ...
        stablePlayerState: StateFlow<com.yuukifst.orpheus.presentation.viewmodel.StablePlayerState>
    ) {
    ...
    fun fetchLyricsForSong(
        ...
        loadingJob = scope?.launch {
            ...
            if (song.isYouTubeSource()) {
                musicRepository.searchRemoteLyricsByQuery(song.title, song.artist)
                    .onSuccess { (query, results) ->
                        if (forcePickResults || results.size > 1) {
                            ...
                            _songUpdates.emit(song.copy(lyrics = result.rawLyrics) to result.lyrics)
                        } else {
                            handleError(NoLyricsFoundException())
                        }
                    }
