package com.yuukifst.orpheus.data.backup.validation
...
import com.yuukifst.orpheus.data.backup.model.Severity
...
class BackupFileValidator @Inject constructor(
    ...
    fun validate(uri: Uri): BackupValidationResult {
        ...
        if (format == BackupFormatDetector.Format.UNKNOWN) {
            errors.add(ValidationError("FORMAT_UNKNOWN", "File is not a recognized Orpheus backup format."))
            return BackupValidationResult.Invalid(errors)
        }
