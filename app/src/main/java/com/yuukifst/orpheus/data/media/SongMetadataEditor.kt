package com.yuukifst.orpheus.data.media
...
import com.yuukifst.orpheus.data.worker.collectArtistNames
...
class SongMetadataEditor(
    ...
    suspend fun writeYouTubeTags(filePath: String, title: String, thumbnailUrl: String) = withContext(Dispatchers.IO) {
        val audioFile = File(filePath)
        if (!audioFile.exists()) return@withContext
        val coverBytes = runCatching {
            java.net.URL(thumbnailUrl).openStream().use { it.readBytes() }
        }.getOrNull()
        ...
        ParcelFileDescriptor.open(audioFile, ParcelFileDescriptor.MODE_READ_WRITE).use { fd ->
            ...
            val existingMetadata = TagLib.getMetadata(metadataFd.detachFd())
            val propertyMap = HashMap(existingMetadata?.propertyMap ?: emptyMap())
            propertyMap["TITLE"] = arrayOf(title)
            val saveFd = fd.dup()
            if (!TagLib.savePropertyMap(saveFd.detachFd(), propertyMap)) return@withContext
            ...
            if (coverBytes != null) {
                ...
                val pictureFd = fd.dup()
                TagLib.savePictures(pictureFd.detachFd(), pictures)
            }
        }
