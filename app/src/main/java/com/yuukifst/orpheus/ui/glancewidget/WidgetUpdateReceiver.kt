...
class WidgetUpdateReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != "com.yuukifst.orpheus.ACTION_WIDGET_UPDATE_PLAYBACK_STATE") return
        ...
        scope.launch {
            try {
                ...
                val glanceIds = glanceAppWidgetManager.getGlanceIds(OrpheusGlanceWidget::class.java)
                glanceIds.forEach { glanceId ->
                    OrpheusGlanceWidget().update(context, glanceId)
                }
